...
export default function AuthPage() {
  ...
  const validate = () => {
    if (!email.trim()) { setError('Please enter your university email.'); return false; }
    if (!isAstonEmail(email)) { setError('Only @aston.ac.uk email addresses are permitted. Verity is exclusive to verified Aston students.'); return false; }
  ...
  const handleForgotPassword = async () => {
    ...
    if (!isAstonEmail(email)) { setError('Please enter your @aston.ac.uk email address first.'); return; }
    setLoading(true);
    ...
    else { setSuccessMsg('Password reset email sent! Check your Aston inbox.'); }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex">
      ...
      >
        ...
        <div className="relative z-10 max-w-md">
          <h1 className="text-6xl text-white font-light tracking-widest mb-3">VERITY</h1>
          <p className="text-white/70 text-lg mb-12">Campus services, built on trust.</p>

          <div className="space-y-4 text-left">
            {[
              { icon: 'fa-solid fa-shield-halved', title: 'Verified Identities', desc: 'Every user is a real Aston student — no anonymous accounts, no strangers.' },
              { icon: 'fa-solid fa-gem', title: 'Trust Orb', desc: 'Visual reputation scores built from structured, attribute-based reviews.' },
              { icon: 'fa-solid fa-star-half-stroke', title: 'Structured Reviews', desc: 'Ratings for punctuality, quality, and communication — not vague star scores.' },
            ].map(({ icon, title, desc }) => (
              <div key={title} className="flex items-start gap-4 p-4 rounded-2xl border border-white/15"
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 bg-neutral-50">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-8">
            <h1 className="text-4xl font-light tracking-widest text-neutral-800 mb-2">VERITY</h1>
            <p className="text-neutral-500 text-sm">Campus services, built on trust.</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-neutral-100 p-8">
            ...
            <div className="flex bg-neutral-100 rounded-xl p-1 mb-8">
              {['Log In', 'Sign Up'].map((tab) => {
                ...
                return (
                  <button key={tab}
                    onClick={() => { setIsLogin(tab === 'Log In'); setError(''); setSuccessMsg(''); }}
                    className="flex-1 py-2.5 px-4 text-sm font-medium rounded-lg transition-all"
                    style={active ? { background: 'white', color: '#171717', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' } : { color: '#737373' }}>
                    {tab}
                  </button>
                );
              })}
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-neutral-500 text-sm">
                {isLogin ? 'Sign in to your student account' : 'Create your verified student account'}
              </p>
            </div>
            ...
            <div className="space-y-5">
              ...
              <button onClick={handleSubmit} disabled={loading}
                ...
                style={{
                  background: loading ? '#a3a3a3' : 'linear-gradient(135deg, #0047AB 0%, #6A0DAD 100%)',
                  cursor: loading ? 'not-allowed' : 'pointer',
                }}>
                {loading ? (
                  <><i className="fa-solid fa-spinner fa-spin mr-2" />{isLogin ? 'Signing in...' : 'Creating account...'}</>
                ) : (
